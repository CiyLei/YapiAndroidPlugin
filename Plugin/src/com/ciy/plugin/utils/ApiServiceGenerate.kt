    ...
    fun createApiService(
        ...
        apiInfoList.forEach {
            try {
                ...
                val responseFile = ResponseModelGenerate.createResponseModel(
                    ResponseModelGenerate.captureName("${urlName}ResModel")
                    , rootDir, modelPackName, it
                )
                // 将分析的data类型套到BaseResponse中
                val baseResponseType = ClassName(packName, ShowInputDialogAction.baseResponseClassName)
                var responseType: TypeName = baseResponseType.parameterizedBy(Any::class.asTypeName())
                if (responseFile != null) {
                    responseType = baseResponseType.parameterizedBy(ClassName(responseFile.packageName, responseFile.name))
                }
                ...
                if (it.method == "GET") {
                    it.req_query.forEach { req_it ->
                        ...
                        val queryAnnotation =
                            AnnotationSpec.builder(queryAnnotationType).addMember("%S", req_it.name).build()
                        // 必传
                        val required = req_it.required == "1"
                        funSpecBuilder.addParameter(ParameterSpec.builder(
                            req_it.name,
                            String::class.asTypeName().copy(!required)
                        )
                    ...
                    val (requestBodyFile, isList) = RequestBodyModelGenerate.createRequestBodyModel(
                        ResponseModelGenerate.captureName("${urlName}ReqModel")
                        , rootDir, modelPackName, it
                    )
                    if (requestBodyFile != null) {
                        ...
                        if (!isList) {
                            funSpecBuilder.addParameter(
                                ParameterSpec.builder("parameter", requestBodyType.copy(true))
                                    .defaultValue("null").addAnnotation(bodyAnnotationType).build()
                            )
                        } else {
                            funSpecBuilder.addParameter(
                                ParameterSpec.builder("parameter", LIST.parameterizedBy(requestBodyType).copy(true))
                                    .defaultValue("null").addAnnotation(bodyAnnotationType).build()
                            )
                        }
                    }
